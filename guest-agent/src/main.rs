...
fn setup_epoll(sfd: BorrowedFd, pty: BorrowedFd) -> io::Result<OwnedFd> {
    let epfd = epoll::create(epoll::CreateFlags::CLOEXEC).unwrap();
    epoll::add(
        &epfd,
        sfd,
        epoll::EventData::new_u64(SignalSource::Signal as u64),
        epoll::EventFlags::IN,
    )?;
    epoll::add(
        &epfd,
        stdin(),
        epoll::EventData::new_u64(SignalSource::Stdin as u64),
        epoll::EventFlags::IN,
    )?;
    epoll::add(
        &epfd,
        pty,
        epoll::EventData::new_u64(SignalSource::Pty as u64),
        epoll::EventFlags::IN,
    )?;
    Ok(epfd)
}
...
fn main() {
    ...
    let epfd = setup_epoll(sfd.as_fd(), pty_fd.as_fd()).unwrap();
    ...
    loop {
        ...
        for ev in events.iter() {
            match SignalSource::from_u64(ev.data.u64()) {
